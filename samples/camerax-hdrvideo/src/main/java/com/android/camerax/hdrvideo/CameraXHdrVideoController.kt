...
private const val TAG = "CameraXHdrVideoController"

@Composable
fun rememberCameraXHdrVideoController(
    context: Context,
 ...
 * 10-bit [androidx.camera.core.DynamicRange]s before binding (via
 * [Recorder.getVideoCapabilities]) and builds its [VideoCapture] with the selected range
 * ([VideoCapture.Builder.setDynamicRange]). Recording at a 10-bit range produces an HDR (HEVC) clip;
 * CameraX drives the encoder. Back/front camera selected at construction.
 */
@Stable
class CameraXHdrVideoController(
    ...
    private var currentRange: HdrDynamicRange = range

    private var cameraControl: CameraControl? = null
    private var cameraProvider: ProcessCameraProvider? = null
    private var activeRecording: Recording? = null

    private var videoCapture: VideoCapture<Recorder> = buildVideoCapture(quality, range)
    ...
    private fun buildVideoCapture(
        ...
        // VideoCapture.Builder (not withOutput) is required to set the dynamic range.
        return VideoCapture
            .Builder(recorder)
            .setDynamicRange(range.range)
            .build()
    }
    ...
    private fun scanSupportedRanges(provider: ProcessCameraProvider): List<HdrDynamicRange> =
        try {
            ...
            val ranges = Recorder.getVideoCapabilities(cameraInfo).supportedDynamicRanges
            HdrDynamicRange.entries
                .filter { it.range in ranges }
                .ifEmpty { listOf(HdrDynamicRange.SDR) }
        } catch (e: Exception) {
            Log.e(TAG, "Failed to scan supported dynamic ranges", e)
            listOf(HdrDynamicRange.SDR)
        }
    ...
    /** Rebinds with a [VideoCapture] configured for the new dynamic [range]. */
    fun updateRange(range: HdrDynamicRange) {
        if (range == currentRange) return
        currentRange = range
        videoCapture = buildVideoCapture(currentQuality, range)
        cameraProvider?.let { bindUseCases(it) }
    }

    /** Rebinds with a [VideoCapture] configured for the new [quality]. */
    fun updateQuality(quality: VideoQuality) {
        if (quality == currentQuality) return
        currentQuality = quality
        videoCapture = buildVideoCapture(quality, currentRange)
        cameraProvider?.let { bindUseCases(it) }
    }
